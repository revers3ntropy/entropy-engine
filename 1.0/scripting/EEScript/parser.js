...
import { N_any, N_array, N_binOp, N_break, N_class, N_continue, N_emptyObject, N_for, N_function, N_functionCall, N_if, N_indexed, N_number, N_objectLiteral, N_return, N_statements, N_string, N_unaryOp, N_undefined, N_varAssign, N_variable, N_while, N_yield } from './nodes.js';
...
export class Parser {
    ...
    atomIdentifier(res, startPos, tok) {
        this.advance(res);
        let node = new N_variable(startPos, this.currentToken.endPos, tok);
        let prevNode = new N_undefined(startPos, this.currentToken.endPos);
        ...
        while ([tt.OPAREN, tt.OSQUARE, tt.DOT].includes(this.currentToken.type)) {
            switch (this.currentToken.type) {
                ...
                case tt.DOT:
                    ...
                    prevNode = node;
                    node = new N_indexed(this.currentToken.startPos, this.currentToken.endPos, node, new N_string(this.currentToken.startPos, this.currentToken.endPos, this.currentToken));
                    this.advance(res);
            }
        }
        if (this.currentToken.type === tt.ASSIGN) {
            ...
            if (node instanceof N_variable) {
                node = new N_varAssign(startPos, this.currentToken.endPos, node.a, value, assignType, false);
            }
    ...
    makeIndex(to) {
        ...
        if (this.currentToken.type !== tt.CSQUARE)
            return res.failure(new InvalidSyntaxError(this.currentToken.startPos, this.currentToken.endPos, "Expected ']'"));
        this.advance(res);
        return res.success(new N_indexed(startPos, this.currentToken.startPos, base, index));
    }
    initiateVar(res, isGlobal, isConstant) {
        ...
        if (this.currentToken.type !== tt.ASSIGN) {
            ...
            return res.success(new N_varAssign(startPos, this.currentToken.startPos, varName, new N_undefined(this.currentToken.startPos, this.currentToken.endPos), '=', isGlobal, 
            // must be false ^
            isConstant));
        }
        let assignType = this.currentToken.value;
        ...
        if (expr instanceof N_class)
            ...
            expr.name = varName.value;
        return res.success(new N_varAssign(startPos, this.currentToken.startPos, varName, expr, assignType, isGlobal, isConstant));
    }
    bracesExp() {
        const res = new ParseResults();
    ...
    classExpr(name) {
        ...
        if (this.currentToken.type === tt.CBRACES) {
            this.advance(res);
            return res.success(new N_class(startPos, this.currentToken.startPos, [], undefined, undefined, name));
        }
        while (true) {
            if (this.currentToken.type !== tt.IDENTIFIER)
                break;
        ...
        return res.success(new N_class(startPos, this.currentToken.startPos, methods, extends_, init, name));
    }
    scriptExpr() {
        const res = new ParseResults();
        const startPos = this.currentToken.startPos;
        ...
        if (!this.currentToken.matches(tt.KEYWORD, 'script'))
            return res.failure(new InvalidSyntaxError(this.currentToken.startPos, this.currentToken.endPos, "Expected 'script'"));
        this.advance(res);
        if (this.currentToken.type === tt.IDENTIFIER) {
        ...
        if (this.currentToken.type === tt.CBRACES) {
            this.advance(res);
            return res.success(new N_ESBehaviour(startPos, this.currentToken.startPos, [], undefined, name));
        }
        while (true) {
            if (this.currentToken.type === tt.OBRACES) {
                ...
                if (this.currentToken.type === tt.ASSIGN) {
                    ...
                    if (res.error)
                        return res;
                    publicVariables.push(new N_objectLiteral(startPos, this.currentToken.startPos, [
                        [
                            new N_any('name'),
                            new N_any(methodId)
                        ],
                    ...
                    return res.failure(new InvalidSyntaxError(this.currentToken.startPos, this.currentToken.endPos, `Expected either '(' or '=', but got `));
            }
            else
                break;
        }
        this.consume(res, tt.CBRACES);
        return res.success(new N_ESBehaviour(startPos, this.currentToken.startPos, methods, undefined, name, publicVariables, entity));
    }
    forExpr() {
        const res = new ParseResults();
        const startPos = this.currentToken.startPos;
        ...
        if (res.error)
            return res;
        this.addEndStatement(res);
        return res.success(new N_for(startPos, this.currentToken.startPos, body, array, identifier, isGlobalIdentifier, isConstIdentifier));
    }
    array() {
        const res = new ParseResults();
        let elements = [];
    ...
    object() {
        ...
        while (true) {
            ...
            if (this.currentToken.type === tt.IDENTIFIER) {
                keyType = 'id';
                key = new N_string(this.currentToken.startPos, this.currentToken.endPos, this.currentToken);
                this.advance(res);
                // @ts-ignore
            }
            ...
            if (this.currentToken.type === tt.COLON) {
                ...
                this.reverse();
                value = new N_variable(this.currentToken.startPos, this.currentToken.endPos, this.currentToken);
                this.advance(res);
                if (this.currentToken.type === tt.COMMA)
                    this.advance(res);
            }
